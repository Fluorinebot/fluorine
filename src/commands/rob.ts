...
export async function run(
    ...
    if (user.id === interaction.user.id) {
        return interaction.reply(
            client.i18n.t('ROB_SELF', { lng: interaction.locale })
        );
    }
    if (!interaction.guild.members.cache.get(user.id)) {
        return interaction.reply(
            client.i18n.t('ROB_INVALID_USER', { lng: interaction.locale })
        );
    }
    const userBalance = await client.economy.get(user.id, interaction.guildId);
    const robberBalance = await client.economy.get(
        interaction.user.id,
        interaction.guildId
    );
    ...
    if (chance > 40) {
        const lost = Math.round(
            (robberBalance.bank + robberBalance.wallet) * (earned / 100)
        );
