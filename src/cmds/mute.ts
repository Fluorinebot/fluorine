...
export async function run(
  ...
  const config: any = await r
    ...
    .run(client.conn);
  if (!message.member?.permissions.has('MANAGE_ROLES')) return message.reply('Nie masz permisji do zmutowania tego użytkownika!');

  if (!message.guild?.me?.permissions.has('MANAGE_ROLES')) return message.reply('Nie posiadam permisji do zarządzania rolami!');
  if (!config.muteRole) return message.reply(
  ...
  const create = await createCase(
    client, message?.guild, member.user, message.author, 'mute', reason
  );
