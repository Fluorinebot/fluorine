...
export async function run(client: FluorineClient, message: Message, args: string[]) {
    ...
    if (!config.muteRole) {
        return message.reply('Nie ustawiono roli do mute, ustaw ją komendą config!');
    }
    if (!args[0]) {
        return message.reply('Musisz podać użykownika');
    }
    ...
    if (!member) {
        return message.reply('Członek którego chcesz odmutować nie istnieje!');
    }
